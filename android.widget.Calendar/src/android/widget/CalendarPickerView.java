...
import android.widget.MonthCellDescriptor.RangeState;
import android.widget.calendar.R;
 ...
 * initialized after inflation with {@link #init(Date, Date)} and can be customized with any of the {@link FluentInitializer} methods returned. The currently selected date can be retrieved with
...
public class CalendarPickerView extends ListView
{
	 ...
	 * of day will be ignored. For instance, if you pass in {@code minDate} as 11/16/2012 5:15pm and {@code maxDate} as 11/16/2013 4:30am, 11/16/2012 will be the first selectable date and
	 * 11/15/2013 will be the last selectable date ({@code maxDate} is exclusive).
	 * <p>
	 * This will implicitly set the {@link SelectionMode} to {@link SelectionMode#SINGLE}. If you want a different selection mode, use {@link FluentInitializer#inMode(SelectionMode)} on the
	 * {@link FluentInitializer} this method returns.
	 * <p>
	 * The calendar will be constructed using the given locale. This means that all names (months, days) will be in the language of the locale and the weeks start with the day specified by the locale.
	 *
	 * @param minDate
	 *        Earliest selectable date, inclusive. Must be earlier than {@code maxDate}.
	 * @param maxDate
	 *        Latest selectable date, exclusive. Must be later than {@code minDate}.
	 */
	 ...
	 * {@link FluentInitializer} this method returns.
	 * <p>
	 * The calendar will be constructed using the default locale as returned by {@link java.util.Locale#getDefault()}. If you wish the calendar to be constructed using a different locale, use
	 ...
	 * be called once, right after the dialog is shown (using {@link android.content.DialogInterface.OnShowListener} or {@link android.app.DialogFragment#onStart()}).
	 */
