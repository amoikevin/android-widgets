...
import android.support.v1.animation.PropertyValuesHolder;
import android.support.v1.animation.ValueAnimator;
...
public class ClockView extends View
{
	...
	private final Matrix matrixCanvas = new Matrix();
	private final Matrix matrixSweepGradient = new Matrix();
	...
	private void rotateCanvas(final Canvas canvas)
	{
		matrixCanvas.reset();
		...
		final int matrixCenterX = centerX;
		final int matrixCenterY = centerY;
	...
	private void translateCanvas(final Canvas canvas, final float x, final float y, final float z)
	{
		matrixCanvas.reset();
		camera.save();
	...
	private void rotateCanvasWhenMove(final float x, final float y)
	{
	...
	private void initBound()
	{
		...
		boundScaleRing = new RectF(centerX - radiusScaleRing, centerY - radiusScaleRing, centerX + radiusScaleRing,
				centerY + radiusScaleRing);
	...
	private void initScaleRing()
	{
		...
		// Parse text baseline
		final float scaleTextWidthTwo = paintNumber.measureText("12");
		final float scaleTextWidthOne = paintNumber.measureText("6");
		final float scaleTextHeight = paintNumber.measureText("12");

		final RectF topTextBound = new RectF();
		...
		final RectF bottomTextBound = new RectF();
		...
		final RectF leftTextBound = new RectF();
		...
		final RectF rightTextBound = new RectF();
		...
		final Paint.FontMetrics fm = paintNumber.getFontMetrics();

		scaleTextDrawCoordinates = new float[4][2];
		scaleTextDrawCoordinates[0][0] = topTextBound.left;
		scaleTextDrawCoordinates[0][1] = topTextBound.top + (topTextBound.bottom - topTextBound.top) / 2
				- (fm.bottom - fm.top) / 2 - fm.top;
		scaleTextDrawCoordinates[1][0] = bottomTextBound.left;
		scaleTextDrawCoordinates[1][1] = bottomTextBound.top + (bottomTextBound.bottom - bottomTextBound.top) / 2
				- (fm.bottom - fm.top) / 2 - fm
				.top;
		scaleTextDrawCoordinates[2][0] = leftTextBound.left;
		scaleTextDrawCoordinates[2][1] = leftTextBound.top + (leftTextBound.bottom - leftTextBound.top) / 2
				- (fm.bottom - fm.top) / 2 - fm.top;
		scaleTextDrawCoordinates[3][0] = rightTextBound.left;
		scaleTextDrawCoordinates[3][1] = rightTextBound.top + (rightTextBound.bottom - rightTextBound.top) / 2
				- (fm.bottom - fm.top) / 2 - fm.top;
	}
	...
	private void initHourPointer()
	{
		...
		final float topX1 = centerX - hpTopEdgeLength / 2;
		final float topY1 = centerY - hpPointerLength + strokeWidthRing / 2;
		final float topX2 = centerX + hpTopEdgeLength / 2;
		final float topY2 = topY1;
		final float topCX1 = centerX;
		final float topCY1 = topY1 - hpTopCYOffset;

		final float bottomX1 = centerX - hpBottomEdgeLength / 2;
		final float bottomY1 = centerY;
		final float bottomX2 = centerX + hpBottomEdgeLength / 2;
		final float bottomY2 = bottomY1;
	...
	private void initMinutePointer()
	{
		...
		final float topX1 = centerX - mpTopEdgeLength / 2;
		final float topY1 = centerY - mpPointerLength + strokeWidthRing / 2;
		final float topX2 = centerX + mpTopEdgeLength / 2;
		final float topY2 = topY1;
		final float topCX1 = centerX;
		final float topCY1 = topY1 - mpTopCYOffset;

		final float bottomX1 = centerX - mpBottomEdgeLength / 2;
		final float bottomY1 = centerY;
		final float bottomX2 = centerX + mpBottomEdgeLength / 2;
		final float bottomY2 = bottomY1;
	...
	private void initTriangleSecondPointer()
	{
		...
		final float height = (float) (1.0 * Math.sqrt(3f) / 2 * triangleSideLength);

		final float point1x = centerX;
		final float point1y = centerY - radius + strokeWidthRing / 2 + trianglePointerOffset;
		final float point2x = point1x - triangleSideLength / 2;
		final float point2y = point1y + height;
		final float point3x = point1x + triangleSideLength / 2;
		final float point3y = point1y + height;
	...
	private void startNewSecondAnim()
	{
		...
		secondAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
		{
			...
			private final float drawInterval = 0.1f;

			private float lastUpdatePointerValue = 0;
			private final float updatePointerInterval = 360 / 60 * 5;

			@Override
			public void onAnimationUpdate(final ValueAnimator animation)
			{
				final float newValue = (float) animation.getAnimatedValue();
	...
	private void startNewSteadyAnim()
	{
		...
		final PropertyValuesHolder holderRotateX = PropertyValuesHolder.ofFloat(propertyNameRotateX, canvasRotateX, 0);
		final PropertyValuesHolder holderRotateY = PropertyValuesHolder.ofFloat(propertyNameRotateY, canvasRotateY, 0);
	...
	private void updateTimePointer()
	{
		...
		final float percentSecond = (float) (1.0 * second / 60);
		final float percentMinute = (float) (1.0 * (minute * 60 + second) / (60 * 60));
		final float percentHour = (float) (1.0 * (hour * 60 * 60 + minute * 60 + second) / (60 * 60 * 12));
	...
	{
		final float degree = (rotateSecondPointer + progressRingInitRotateDegree) % 360;
		return degree;
	}
