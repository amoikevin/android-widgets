...
import android.support.v1.animation.Animator;
...
import android.widget.shimmerframelayout.R;
...
public class ShimmerFrameLayout extends FrameLayout
{
	...
	/**
	 * Get the shape of the current animation's highlight mask. One of {@link MaskShape#LINEAR} or {@link MaskShape#RADIAL}
	 *
	 ...
	 *
	 * @param shape
	 *        The shape of the highlight mask
	 */
	 ...
	 * <p/>
	 * It is the relative distance from the center at which the highlight mask's opacity is 0 i.e it is fully transparent. For a linear mask, the distance is relative to the center towards the edges.
	 * For a radial mask, the distance is relative to the center towards the circumference. So a dropoff of 0.5 on a linear mask will create a band that is half the size of the corresponding edge
	 * (depending on the {@link MaskAngle}), centered in the layout.
	 *
	 * @param dropoff
	 */
	 ...
	 * <p/>
	 * Intensity is the point relative to the center where opacity starts dropping off, so an intensity of 0 would mean that the highlight starts becoming translucent immediately from the center (the
	 * spread is controlled by 'dropoff').
	 *
	 * @param intensity
	 *        The intensity of the highlight mask.
	 */
	 ...
	 * <p/>
	 * If it fails with an OutOfMemory error, it will force a GC and then try to create the bitmap one more time.
	 *
	 * @param width
	 *        width of the bitmap
	 * @param height
